...
CONDA_APPTAINER_IMAGE=$(readlink -f $(dirname $BASH_SOURCE)/miniforge-26.sif)

function vea() {
    #set -x
    local BASE PATH_IN PATH_OUT BIND_ENV
    BASE=venva
	...
	echo apptainer exec --no-home $BIND_ENV --env LANG= "$IMG" 'bash -c "source /venv-apptainer/bin/activate ; \${@:-bash}" - "$@"' > $BASE/exec
    elif [ "$install_type" = conda ] ; then
	echo apptainer exec --no-home $BIND_ENV --env LANG= "$IMG" 'bash -c "source activate /venv-apptainer ; \${@:-bash}" - "$@"' > $BASE/exec
    fi
